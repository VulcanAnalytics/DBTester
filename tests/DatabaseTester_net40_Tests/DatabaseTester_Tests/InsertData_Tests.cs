...
namespace VulcanAnalytics.DBTester.dbSpecflow_tests.DatabaseTester_Tests
{
    ...
    public class InsertData_Tests
    {
        ...
        [TestMethod]
        public void Can_Insert_Into_Column_With_Quoted_Spaces_In_Name()
        {
            var expectedCount = 2;
            var schemaName = "dbo";
            var tableName = "testtable";
        ...
        [TestMethod]
        public void Can_Insert_Into_Table_With_Quoted_Spaces_In_Name()
        {
            var expectedCount = 2;
            var schemaName = "dbo";
            var tableName = "[test table]";
        ...
        [TestMethod]
        public void Can_Insert_Into_Table_With_Quoted_Spaces_In_Schema_Name()
        {
            var expectedCount = 2;
            var schemaName = "[test schema]";
            var tableName = "testtable";
        ...
        private void DropAndCreateTestTable(string schemaName, string tableName, string columnDef)
        {
            ...
            if (!tester.HasSchema(tester.UnquotedIdentifier(schemaName)))
            {
        ...
        {
            var dropTableStatement = string.Format("drop table if exists {0}.{1};", tester.QuotedIdentifier(schemaName), tester.QuotedIdentifier(tableName));
        ...
        {
            var createTableStatement = string.Format("create table {0}.{1}({2});", tester.QuotedIdentifier(schemaName), tester.QuotedIdentifier(tableName),columnDef);
            tester.ExecuteStatementWithoutResult(createTableStatement);
        }

        private void CreateTestSchema(string schemaName)
        {
            var createSchemaStatement = string.Format("create schema {0};", tester.QuotedIdentifier(schemaName));
